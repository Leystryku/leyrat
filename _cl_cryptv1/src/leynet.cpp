...
char *leynet_udp::Receive(int*msgsize, char*buffer, int buffersize, char*ip, short recport)
{
	...
	*msgsize = ret;
	...
	if (recport != -1)
		recport = (int)ntohs(from.sin_port);
...
char* leynet_tcp::Bind(short port)
{
...
char* leynet_tcp::Listen(char*ip, short&port, unsigned int*ssock)
{
...
char* leynet_tcp::OpenConnection(char*addr, short port)
{
...
char *leynet_tcp::HTTPParseLength(int*msgsize, char*msg)
{
	...
	for (foundlength; foundlength < size; foundlength++)
	{
		if (!memcmp(msg + foundlength, "Content-Length:", 15))
		{
			bfoundlength = true;
			break;
		}
...
char *leynet_tcp::Receive(int* msgsize, char*buffer, int buffersize, TCP_Finished fin)
{
	...
	while (true)
	{
		...
		if (totaldatalen > buffersize)
			break;

		memset(curbuffer, 0, curbuffer_size);

		//int curdatalen = recv(sock, buffer + totaldatalen, buffersize - 1, 0);
		int curdatalen = recv(sock, curbuffer, curbuffer_size-1, 0);
		if (curdatalen > 0)
		{
			...
			if (fin)
			{
				bool is_fin = fin(totaldatalen, curdatalen, buffer, curbuffer);
...
bool leynet_tcp::TLenFin(int datalen, int curdatalen, char*buffer, char*curbuffer)
{
...
bool leynet_tcp::THTTPLenFin(int datalen, int curdatalen, char*buffer, char*curbuffer)
{
